...
export default function OrdersPage() {
  ...
  // Payment state
  const [confirmingOrder, setConfirmingOrder] = useState<{ [key: number]: boolean }>({});
  ...
  const handleConfirmOrder = async (order: any) => {
    ...
    try {
      ...
      const contractAbiRaw = process.env.NEXT_PUBLIC_MAIN_CONTRACT_CONFIRME_ORDER_ABI || "";
      ...
      if (receipt.status === 'success') {
        ...
        try {
          ...
          setMessageText(`Order confirmation failed: ${backendError.message || 'Unknown error'}`);
          message.error(`Order confirmation failed: ${backendError.message || 'Unknown error'}`);
        }
          ...
          <div style={{ 
            ...
            >
              {confirmingOrder[order.id] ? (
                ...
                `Pay Final Amount (${formatAmount((parseFloat(order.total_amount) * 0.85).toString())} ETH)`
              )}
            </Button>
          </div>
        )}
