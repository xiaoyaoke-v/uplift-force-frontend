...
import { TrophyOutlined, CheckCircleOutlined, MessageOutlined, RedoOutlined, StarOutlined, ClockCircleOutlined, CloseOutlined, CopyOutlined } from '@ant-design/icons';
import { useUser } from '@/contexts/UserContext';
import { useRouter } from 'next/navigation';
import Header from '@/components/Header';
import Sidebar from "@/components/Sidebar";
import { getMyOrders, getAllOrders, type IOrder, type IOrdersParams, acceptOrder, completeOrder, cancelOrder } from '@/apis';
import { mainnet, sepolia, hardhat, arbitrum } from 'viem/chains';
import { useAccount, useSignMessage } from "wagmi"
import { parseEther, parseAbi, createPublicClient, createWalletClient, custom, BaseError, ContractFunctionRevertedError  } from "viem"
...
export default function OrdersPage() {
  ...
  const [completingOrder, setCompletingOrder] = useState<{ [key: number]: boolean }>({});
  const [cancellingOrder, setCancellingOrder] = useState<{ [key: number]: boolean }>({});
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const [successModalType, setSuccessModalType] = useState<'accept' | 'complete'>('accept');
...
const fetchAllOrders = async () => {
  ...
  const handleConfirmCancelOrder = async () => {
    if (!orderToCancel || !publicClient || !walletClient || !address) {
      ...
      message.error("Unable to cancel order. Please check your wallet connection.");
      return;
    }
    ...
    try {
      ...
      const blockchainOrderId = orderToCancel.chain_order_id || orderToCancel.blockchain_order_id || orderToCancel.id;
      ...
      if (receipt.status === 'success') {
        setMessageText("Cancellation transaction confirmed! Updating order status...");
  ...
  // 监听合约事件的函数
const listenToContractEvents = async (orderId: number, functionsTxHash: string) => {
  ...
  try {
        ...
        for (const log of logs) {
          try {
            ...
            if (log.eventName === 'OrderCompleted') {
              ...
              const boosterRewardEth = (Number(boosterReward) / 1e18).toFixed(6);
              
              setMessageText(`Order completed successfully! Booster reward: ${boosterRewardEth} ETH`);
              message.success('Order completed successfully! Payment processed.');
              
              // 调用后端接口更新订单状态为完成
              try {
                await completeOrder(
                  orderId, 
                  `Order completed via blockchain automation. Platform fee: ${platformFeeEth} ETH, Booster reward: ${boosterRewardEth} ETH. Functions TX: ${functionsTxHash}, Settlement TX: ${settlementTxHash}`,
                  settlementTxHash,
                  'completed'
                );
                ...
                console.error('Failed to update backend order status:', backendError);
                message.error('Order completed on blockchain but failed to update backend status');
                setMessageText('Order completed on blockchain but failed to update backend status');
              } finally {
                // 🔥 确保在完成后重置按钮状态
              ...
              const { playerRefund, penaltyToPlayer, penaltyToPlatform, currentTxHash } = log.args;
              
              const playerRefundEth = (Number(playerRefund) / 1e18).toFixed(6);
              const penaltyToPlayerEth = (Number(penaltyToPlayer) / 1e18).toFixed(6);
              const penaltyToPlatformEth = (Number(penaltyToPlatform) / 1e18).toFixed(6);
              
              setMessageText(`Order failed! Player refund: ${playerRefundEth} ETH. Penalties applied.`);
              message.error('Order verification failed! Refund and penalties processed.');
              
              // 调用后端接口更新订单状态为失败
              try {
                await completeOrder(
                  orderId,
                  `Order failed via blockchain automation. Player refund: ${playerRefundEth} ETH, Penalty to player: ${penaltyToPlayerEth} ETH, Penalty to platform: ${penaltyToPlatformEth} ETH. Functions TX: ${functionsTxHash}, Settlement TX: ${settlementTxHash}`,
                  settlementTxHash,
                  'failed'
                );
              } catch (backendError) {
                console.error('Failed to update backend order failure status:', backendError);
                message.error('Order failed on blockchain but failed to update backend status');
                setMessageText('Order failed on blockchain but failed to update backend status');
              } finally {
                // 🔥 确保在失败后重置按钮状态
                setCompletingOrder(prev => ({ ...prev, [orderId]: false }));
              }
    ...
    setTimeout(() => {
      ...
      setMessageText('Verification timeout. Please refresh to check order status.');
      // 🔥 超时后重置按钮状态
      setCompletingOrder(prev => ({ ...prev, [orderId]: false }));
    }, 150000);
...
const handleCompleteOrder = async (order: any) => {
  ...
  try {
    ...
    const contractAbiRaw = process.env.NEXT_PUBLIC_QUERY_RIOT_CONTRACT_COMPLETE_ORDER_ABI || "";
    ...
    const blockchainOrderId = order.chain_order_id || order.blockchain_order_id || order.id;
    ...
    try {
      ...
      setMessageText("Transaction simulation failed. Please check the order status.");
      setCompletingOrder(prev => ({ ...prev, [orderId]: false }));
      return;
    }
    ...
    setMessageText(`Completion request sent: ${txHash}. Waiting for verification...`);
    ...
    if (receipt.status === 'success') {
      setMessageText("Verification request confirmed! Waiting for Chainlink Functions to process and complete the order...");
  ...
  try {
    const contractAddr = process.env.NEXT_PUBLIC_MAIN_CONTRACT_ADDR;
    const contractAbiRaw = process.env.NEXT_PUBLIC_MAIN_CONTRACT_ACCEPT_ORDER_ABI || "";
    ...
    setMessageText(`Transaction sent: ${txHash}. Waiting for confirmation...`);
    ...
    if (receipt.status === 'success') {
      ...
      try {
        ...
        setMessageText(`Order acceptance failed: ${backendError.message || 'Unknown error'}`);
        message.error(`Order acceptance failed: ${backendError.message || 'Unknown error'}`);
      }
        ...
        <div style={{
          ...
          {activeTab === 'my' ? (
            <>
              ...
              {order.status === 'in_progress' && order.my_role === 'booster' && (
                <Button
                  size="small"
                  ...
                  style={{
                    ...
                    cursor: completingOrder[order.id] ? 'not-allowed' : 'pointer',
                  }}
                >
      ...
      >
        <div style={{
          ...
          <p style={{
            ...
            Cancelling this order will result in your deposit being forfeited as a penalty!
          </p>
        ...
        >
          <div
            style={{
              ...
              `,
              animation: 'successPulse 0.6s ease-out, float 3s ease-in-out infinite',
              minWidth: '300px',
              overflow: 'hidden'
            }}
          >
            ...
            >
              {successModalType === 'accept' ? 'ORDER ACCEPTED!' : 'ORDER COMPLETED!'}
            </div>
            ...
            >
              {successModalType === 'accept' ? 'Transaction Confirmed' : 'Verification Complete'}
            </div>
  ...
  >
    ...
    >
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: 'left' }}>
            <div style={{ 
              ...
              {selectedContactOrder.my_role === 'player' ? 'Booster' : 'Client'}
            </div>
      ...
      >
        <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 'bold' }}>{selectedContactOrder.game_type}</div>
          </div>
          
          {selectedContactOrder.current_rank && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: '#888', marginBottom: '3px' }}>From</div>
              <div style={{ fontWeight: 'bold' }}>{selectedContactOrder.current_rank}</div>
            </div>
          )}
          ...
          {selectedContactOrder.target_rank && (
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontWeight: 'bold' }}>{selectedContactOrder.target_rank}</div>
            </div>
          )}
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 'bold', color: '#00ff00' }}>
              {formatAmount(selectedContactOrder.total_amount)} ETH
            </div>
          </div>
        </div>
      </div>
      ...
      >
        📧 Use this email to coordinate order details, share game credentials, 
        and communicate progress updates. Keep all communication professional 
        and focused on the boost service.
      </div>
